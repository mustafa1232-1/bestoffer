...
class _AdminAdBoardScreenState extends ConsumerState<AdminAdBoardScreen> {
  ...
  List<AdBoardItemModel> _applyTab(List<AdBoardItemModel> items) {
    ...
    switch (_tab) {
      ...
      case _AdTab.marketplace:
        return items.where((i) => i.placement == 'MARKETPLACE_HOME').toList();
      case _AdTab.categories:
        return items
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Wrap(
                                ...
                                children: [
                                  _Chip(
                                    label: 'مشاهدات: ${item.impressionCount}',
                                  ),
            ...
            children: [
              if (imageUrl.isNotEmpty)
                CachedAppImage(
                  ...
                  fit: BoxFit.cover,
                  errorWidget: (_, _, _) =>
                      const ColoredBox(color: Color(0xFF1A2E59)),
                )
              else
                const DecoratedBox(
                  decoration: BoxDecoration(
...
class _AdCategoryTarget {
  ...
  String label(BuildContext context) => context.lt(ar: labelAr, en: labelEn);
...
class _AdBoardSheetState extends ConsumerState<_AdBoardSheet> {
  ...
  void initState() {
    ...
    _activityTypeCtrl = TextEditingController(
      text: initial?.activityType ?? '',
    );
    ...
    _ctaType = _normalizedActionType(
      (initial?.type ?? initial?.ctaTargetType ?? 'none').trim(),
    );
  ...
  String _normalizedActionType(String value) {
  ...
  void _setActionType(String value) {
    setState(() {
      _ctaType = _normalizedActionType(value);
      if (_ctaType != 'product') {
        _selectedProductId = null;
      }
      if (_ctaType != 'store_ad' && _ctaType != 'product') {
        _merchantId = null;
        _products = const [];
      }
      if (_ctaType == 'category_ad' && _selectedCategoryTarget == null) {
        _selectedCategoryTarget = _categoryTargets.first;
        _categoryCtrl.text = _selectedCategoryTarget?.category ?? '';
        _activityTypeCtrl.text = _selectedCategoryTarget?.activityType ?? '';
      }
    });
  }

  void _syncActionControllers() {
    switch (_ctaType) {
      case 'store_ad':
        final merchant = _merchantId?.toString() ?? '';
        _targetIdCtrl.text = merchant;
        _ctaValueCtrl.text = merchant;
        _targetRouteCtrl.clear();
        _externalLinkCtrl.clear();
        _promoCodeCtrl.clear();
        break;
      case 'product':
        _targetIdCtrl.text = _merchantId?.toString() ?? '';
        _ctaValueCtrl.text = _selectedProductId?.toString() ?? '';
        _targetRouteCtrl.clear();
        _externalLinkCtrl.clear();
        _promoCodeCtrl.clear();
        break;
      case 'category_ad':
        ...
        _externalLinkCtrl.clear();
        _promoCodeCtrl.clear();
        _ctaValueCtrl.text = _activityTypeCtrl.text.trim().isNotEmpty
            ? _activityTypeCtrl.text.trim()
      ...
      case 'external_link':
        ...
        _targetRouteCtrl.clear();
        _promoCodeCtrl.clear();
        _ctaValueCtrl.text = _externalLinkCtrl.text.trim();
        break;
      case 'internal_route':
        _merchantId = null;
        _selectedProductId = null;
        ...
        _externalLinkCtrl.clear();
        _promoCodeCtrl.clear();
        _ctaValueCtrl.text = _targetRouteCtrl.text.trim();
        break;
      case 'taxi':
      case 'none':
      default:
  ...
  bool get _actionNeedsMerchant =>
      _ctaType == 'store_ad' || _ctaType == 'product';

  bool get _actionNeedsProduct => _ctaType == 'product';

  bool get _actionNeedsCategory => _ctaType == 'category_ad';
  ...
  Future<void> _submit() async {
    if (_saving) return;
    _syncActionControllers();
    if (!(_formKey.currentState?.validate() ?? false)) return;
    if (_actionNeedsMerchant && _merchantId == null) {
    ...
    if (_actionNeedsProduct && _selectedProductId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            context.l10n.adminAdBoardChooseMerchantAndProductFirst,
    ...
    try {
      final payload = <String, dynamic>{
        ...
        'activityType':
            (_placement == 'MARKETPLACE_CATEGORY' || _ctaType == 'category_ad')
  ...
  bool get _useSimplifiedAdComposer => true;

  List<Widget> _simplifiedAdFields(List<DropdownMenuItem<int?>> merchants) {
    return [
      DropdownButtonFormField<String>(
        initialValue: _placement,
        isExpanded: true,
        decoration: InputDecoration(
          labelText: context.lt(ar: 'مكان الإعلان', en: 'Ad placement'),
        ),
        items: _placements
            .map(
        ...
        onChanged: (value) {
          ...
          if (_placement == 'MARKETPLACE_CATEGORY' &&
            ...
            _applyCategoryTarget(_categoryTargets.first);
          }
        },
      ),
      if (_placement == 'MARKETPLACE_CATEGORY' || _actionNeedsCategory) ...[
        const SizedBox(height: 8),
        DropdownButtonFormField<_AdCategoryTarget?>(
          ...
          decoration: InputDecoration(
            labelText: context.lt(
              ar: 'القسم أو التصنيف',
              en: 'Section or category',
            ),
            helperText: context.lt(
              ar: 'يحدد مكان ظهور الإعلان في صفحات الأقسام أو التصنيف الذي يفتحه الزر.',
              en: 'Controls where the ad appears or which category the button opens.',
            ),
          ),
      ...
      _ImageField(
        ...
        onPick: _pickImage,
        onClear:
            (_imageFile != null || (widget.initial?.imageUrl ?? '').isNotEmpty)
            ? () {
                setState(() {
                  _imageFile = null;
                  _imageUrlCtrl.clear();
      ...
      TextFormField(
        ...
        decoration: InputDecoration(labelText: context.l10n.adminAdBoardTitle),
      ...
      DropdownButtonFormField<String>(
        ...
        onChanged: (value) => _setActionType(value ?? 'none'),
      ),
      if (_actionNeedsMerchant) ...[
        const SizedBox(height: 8),
        DropdownButtonFormField<int?>(
          ...
          validator: (value) {
            if (_actionNeedsMerchant && value == null) {
              return context.l10n.adminAdBoardRequiredField;
            }
            return null;
          },
      ...
      if (_actionNeedsProduct && _merchantId != null) ...[
        const SizedBox(height: 8),
        DropdownButtonFormField<int?>(
          initialValue: _products.any((item) => item.id == _selectedProductId)
              ? _selectedProductId
              : null,
          items: _products
              .map(
          ...
          validator: (value) {
            if (_actionNeedsProduct && value == null) {
              return context.l10n.adminAdBoardRequiredField;
            }
            return null;
          },
      ...
      if (_ctaType == 'internal_route') ...[
        ...
        TextFormField(
          ...
          decoration: const InputDecoration(labelText: 'Internal Route'),
          validator: (value) {
      ...
      if (_ctaType == 'external_link') ...[
        ...
        TextFormField(
          ...
          decoration: const InputDecoration(labelText: 'External Link (https)'),
          validator: (value) {
      ...
      const SizedBox(height: 12),
      _SectionLabel(
        text: context.lt(ar: 'معاينة الإعلان', en: 'Ad preview'),
      ),
      const SizedBox(height: 6),
      _AdMobilePreview(
        ...
        child: Form(
          ...
          child: ListView(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              if (_useSimplifiedAdComposer)
                ..._simplifiedAdFields(merchants)
              else ...[
                TextFormField(
                  controller: _titleCtrl,
                  decoration: InputDecoration(
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: [
                    ...
                    DropdownMenuItem(
                      ...
                      child: Text(
                        context.l10n.adminAdBoardInternalCampaignPage,
                      ),
                    ),
